                    ...
                    <table className="table table-striped text-left">
                      <thead>
                        <tr>
                          ...
                          <th scope="col" width="10%">Used Active</th>  {/* New column */}
                        </tr>
                      </thead>
                      <tbody>
                        {initLoading === true ? (
...
const Row = ({row, index,  selectProduct}) => {
  ...
  const [PTU_ACTIVE, setPTUACTIVATE] = useState(row.PTU_ACTIVE);
  const { apiURL, gdcURL, token, placeHolderImageURL } = useContext(ConfigContext);
  ...
  const UpdateActiveUsedStatusProduct = (status) => {
    var formData = {
      PTU_ACTIVE: status,
      PT_ID: row.PT_ID,
    }
    ...
    axios
      .post(`${gdcURL}Parts/UpdateActiveUsedStatusProduct`, formData, { headers })
      .then((response) => {
        if (response.data.success === true) {
          Swal.fire({ title: '<strong>Success</strong>', html: response.data.message, icon: 'success' })
        } else {
    ...
    return <tr>
        ...
        <td>
          <input 
            checked={PTU_ACTIVE === 1} 
            onClick={(e) => {
              setPTUACTIVATE(e.target.checked ? 1 : 0);
              UpdateActiveUsedStatusProduct(e.target.checked ? 1 : '0');
            }} 
            className="form-check-input code-switcher" 
            type="checkbox" 
            id="used-gradient-button" 
          />
        </td>
    </tr>
}
